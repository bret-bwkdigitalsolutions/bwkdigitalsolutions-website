...
## What This Is
A static marketing website for BWK Digital Solutions, a Little Rock, Arkansas consultancy founded by Bret Kramer that helps businesses cut through digital complexity via automation, systems integration, web development, and applied AI. The site is the company's primary customer-facing surface and lead-generation front door, built around the positioning line "Technology Should Solve Problems, Not Create Them," and it doubles as a content/SEO play through an attached blog plus metadata explicitly targeting AI/LLM crawlers, not just traditional search engines.
...
- `/about.html`, `/services.html`, `/contact.html` — standalone pages covering the same topics as the homepage anchor sections; the relationship between the two (duplicate, deprecated, planned split) is not evident from the code
...
## Recent Activity
Over the past several weeks, the only commits to land are recurring "regenerate living state" documentation updates — no HTML, CSS, or workflow changes accompany them. The last substantive change to the site itself was publishing a single blog post in early June ("Why Your AI Isn't Saving You as Much Time as It Should"). Since then the project has sat in a full holding pattern: roughly a dozen consecutive commits, all documentation-only. The last real building happened in mid-April, in a tight cluster: blog infrastructure went in (post template, publish command, weekly draft cron), followed by the contact form, process section, and bird origin story, followed by design polish (staggered hero entrance, navbar scroll state, section refinement). That April work produced the tooling that eventually shipped the June post, but the weekly cadence implied by "weekly draft cron" has not materialized — one post in three-plus months of that tooling existing. Right now, momentum on the site itself is at zero; the only thing running on a schedule is the living-state regeneration.
...
- Blog has exactly one post despite weekly-draft cron tooling being in place for over three months — the implied cadence has not been realized
...
- The living-state doc has kept auto-regenerating on schedule for weeks with nothing new to report — a clear sign the automation is running well ahead of actual project activity
...
- Why has the April weekly draft cron produced only one published post — is it not firing, or is content being drafted and never published?
